...
function stubGet(impl) {
    const get = impl ?? jest.fn().mockResolvedValue({ ok: true, value: 'OK' });
    mockedCreateGetApi.mockReturnValue({ get });
    return get;
}
function stubPost(impl) {
    const post = impl ?? jest.fn().mockResolvedValue({ ok: true, value: 'OK' });
...
describe('useApiFactory', () => {
    ...
    it('POST', async () => {
        ...
        const { result } = renderHook(() =>
            useApiFactory('post', 'https://api', '/r'),
        );
        await expect(result.current()).resolves.toEqual({
            ok: true,
            value: 'POST',
