...
import { SecureStorage, TokenData, UserCredentials } from '@core/domain';
...
function createSecureStorage<T>(
    ...
    const save = async (data: T): Promise<void> => {
        try {
            ...
            log.error('Erreur lors de la sauvegarde sécurisée', {
                error,
                service,
            });
        }
    ...
    const get = async (): Promise<T | undefined> => {
        try {
            ...
            log.error('Erreur lors de la lecture sécurisée', {
                error,
                service,
            });
            return undefined;
